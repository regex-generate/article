...
us to drive the evaluation of the sequence lazily. With this definition, \code{union s1 s2}
...
which available in \ocaml as the \code{Seq} module.
...
The only novel operation is \code{append} to compute the product of two sets.
Since we do not store values at the leaves,
it can be implemented in a single traversal which will graft the appended trie
\code{t0} at each leaf of \code{t}, without copying it.
