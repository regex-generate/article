...
As such, we can implement tries similarly to tries of integers as described
by \citet{Okasaki98fastmergeable}.
For the sake of simplicity, we decide not to use path compression, which mean
that branches will always contain only one char.
A trie is then either empty, a leaf containing a value or a Node containing a map from characters
to children of the trie. Since the only value we are interested in are the path themselves, we use tries with unit values. 
...
The implementation of most operations follow the literature closely. The only
novel operation is \code{append}, which can be implemented using a simple
recursive walk. Note how the value t0 is not copied, thus enabling
a high degree of sharing between the tries.
...
This implementation of append relies on the fact that we do not store
values at the leaf. If we were to store, for instance, the path to the leaf,
we would have to modify all the leafs of the tries. This would be both
expansive to compute and lose the sharing among tries at the leaves.
