...
evaluates to a finite representation:
\begin{lstlisting}[numbers=none]
Cons (Univ [T.empty]) (Cons (Data [T.singleton 'b']) (Full _))
\end{lstlisting}
...
detecting finiteness in Section~\ref{sec:segm-repr}. Consider
languages $X, Y \subseteq \Sigma^*$ and 
suppose that there exist numbers $f_x, f_y\ge0$ such that
\begin{align*}
  \forall x \in \Sigma^*, & |x| \ge f_x \Rightarrow x \in X
  &\forall y \in \Sigma^*, & |y| \ge f_y \Rightarrow y \in Y
\end{align*}
In this case, we can show that $\forall w \in \Sigma$ if $|w|\ge
f_x+f_y$, then $w \in X\cdot Y$.
