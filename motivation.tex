...
in~\eqref{eq:1}. The output of \code{combine} is strictly increasing
...
The function \code{collect} implements the summation in~\eqref{eq:1}
...
To compute the Kleene closure, we can apply the same ideas as for
...
$\setminus$ via the \code{difference} operator in
Figure~\ref{fig:merging-lists}.  If \code{lx} is the LLO
